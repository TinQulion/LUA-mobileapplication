...
local function changeScenes()
    WDvalues = pickerWheel:getValues()
    lastWDvalue = WDvalues[1].value
   -- composer.setVariable("lastWDvalue", WDvalues[1].value)
    --print (lastWDvalue, lastCDvalue)
    composer.gotoScene("food")
    composer.setVariable("lastWDvalue", WDvalues[1].value)
end
...
function scene:create( event )
    ...
    bg = display.newImage("hotdrink.png",display.contentCenterX, display.contentCenterY/8)
    sceneGroup:insert(bg)

    title = display.newText("Varma drycker", display.contentCenterX, display.contentCenterY, display.contentWidth/2, display.contentHeight*.8, "Helvetica", 30)
    title:setFillColor(1,0,0)
    sceneGroup:insert(title)

    buttonText = display.newText("Gå till välj fika",display.contentCenterX*2, display.contentCenterY,display.contentHeight,display.contentWidth/6, "Helvetica", 15)
